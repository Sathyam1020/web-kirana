...
describe("POST /v1/addresses", () => {
  ...
  it("second address: isDefault=false unless explicitly requested", async () => {
    const customer = await signupCustomer(app)
    await api().post("/v1/addresses").set("Cookie", customer.cookieHeader).send(baseAddress)
...
describe("GET /v1/addresses", () => {
  ...
  it("default first, then createdAt DESC", async () => {
    ...
    const list = await api().get("/v1/addresses").set("Cookie", customer.cookieHeader)
  ...
  it("cross-customer isolation: B never sees A's addresses", async () => {
    ...
    await api().post("/v1/addresses").set("Cookie", customerA.cookieHeader).send(baseAddress)
    const customerB = await signupCustomer(app, "B")
    const list = await api().get("/v1/addresses").set("Cookie", customerB.cookieHeader)
...
describe("GET /v1/addresses/:id", () => {
  ...
  it("404 for someone else's address id (no enumeration)", async () => {
    const customerA = await signupCustomer(app, "A")
    const create = await api().post("/v1/addresses").set("Cookie", customerA.cookieHeader).send(baseAddress)
    const customerB = await signupCustomer(app, "B")
    const res = await api()
      .get(`/v1/addresses/${create.body.data.address.id}`)
...
describe("POST /v1/addresses/:id/default", () => {
  it("clears the prior default and sets this one", async () => {
    const customer = await signupCustomer(app)
    const a = await api().post("/v1/addresses").set("Cookie", customer.cookieHeader).send({ ...baseAddress, label: "A" })
    const b = await api().post("/v1/addresses").set("Cookie", customer.cookieHeader).send({ ...baseAddress, label: "B" })
  ...
  it("idempotent on already-default", async () => {
    const customer = await signupCustomer(app)
    const create = await api().post("/v1/addresses").set("Cookie", customer.cookieHeader).send(baseAddress)
